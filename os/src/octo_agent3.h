#ifndef _OCTO_AGENT_H_
#define _OCTO_AGENT_H_
...
#include <stdio.h>
  /**
 * \defgroup agent_grp Agent / OSAL: Functions to create and interact with Agents, Constraints and "high level" Claims.
 * @{
 */
 ...
 */
  ...
  /**
 * \brief Gets the operating point index which was satisfied.
 *
 * \param claim AbstractAgentOctoClaim handle (which is either castable to a AgentOctoClaim* via asAOC() XOR to a ProxyAgentOctoClaim* via asPAOC())
 *
 * \returns The operating point index which was satisfied. Return value 100 (Not Found) if the claim is not generated using Actor Solver.
 */
 ...
 */
  proxy_claim_t agent_claim_get_proxyclaim_tile_type(agentclaim_t claim, tile_id_t tileID, res_type_t type);
 ...
 *
 * \note Constraints are owned by the application, not iRTSS.
 * The application is responsible to delete all constraints it creates.
 * Constraints must not be deleted as long as an agent claim uses it.
 *
 ...
 */
  void agent_constr_set_quantity(constraints_t constr, unsigned int minPEs, unsigned int maxPEs, res_type_t type); // sets the PE-Quantity Constraint (min, max)
 ...
 *
 * UPDATE: 	To specify more exact values for the average parallelism, the function
 * 			also requires A to be scaled by 100 like the sigma value.
 *
 * \param constr Constraints handle for which this constraint should be set.
 * \param A      average parallelism * 100
 ...
 * This only sets values for first 32 Tiles. If there are bigger systems in the future it has to be adjusted.
 *
 * \param constr Constraints handle for which this constraint should be set.
 ...
 *
 * \param constr Constraints handle for which this constraint should be set.
 * \param sticky Turns the constraint off if 0, on otherwise.
 */
 ...
 */
  ...
  /**
 * \brief Sets the 'LocalMemory' constraint.
 *
 * \param constr Constraints handle for which this constraint should be set.
 * \param min sets the minimum amount of local memory per PE
 * \param max sets the maximum amount of local memory per PE
 */
  void agent_constr_set_local_memory(constraints_t constr, int min, int max);

  /**
 * \brief Sets the 'SearchLimit' constraint.
 *
 * \param constr Constraints handle for which this constraint should be set.
 * \param timeLimit sets the time after which the solving algorithm aborts, in milliseconds.
 * 					If a solution is found till than, it is returned.
 * 					The allowed value range is between 500 and 600000.
 */
 ...
 *
 * \param constr Constraints handle for which this constraint should be set.
 * \param ratingLimit sets the percentage of the maximum possible rating at which the solving algorithm stops.
 * 						The allowed value range is betwee 50 and 100.
 */
...
*/
  ...
  /**
 * \brief creates an actor constraint with the three parameters
 * \param number_of_clusters : the maximal number of clusters the actor constraint can contain
 * \param number_of_cluster_guarantees : the maximal number of cluster guarantees the actor constraint can contain
 * \param number_of_operating_points : the maximal number of operating points the actor constraint can contain
 * \returns return the created actor constraint
 */
 ...
 * \param actorConstraint: the actor constraint we want to add a new cluster to. The cluster get an ID which correspond to the index position he was added in the array cluster
 * \param type : the type of the cluster which should be added to the constraint (RISC, ICORE, TCPA)
 * \return void
 */
 ...
 * \param actorConstraint: the actor constraint we want to add a new cluster to. The cluster guarantee get an ID which correspond to the index position he was added in the array of cluster guarantees
 * \param firstClusterType: The first of the two clusters we want a guarantee between, the cluster should belong to the cluster list of the actor constraint it's added to
 * \param secondClusterType: The second of the two clusters we want a guarantee between, the cluster should belong to the cluster list of the actor constraint it's added to
 * \param hopDistance: the maximal hop count we need between the first and the second clusters
 * \param serviceLevel: the minimum service level we need between the first and the second clusters
 * \return void
 */
 ...
 * \param actorConstraint: the actor constraint we want to add a new operating point to. The cluster guarantee get an ID which correspond to the index position he was added in the array of cluster guarantees
 * \param number_of_cluster_guarantees: the maximal number of cluster guarantees the operating point should contain
 * \param number_of_clusters: the maximal number of clusters the operating point should contain
 * \return void
 */
  ...
  /**
 * \brief Add a new operating point to the Actor Constraint.The operating point will get the specified cluster id added in his cluster list
 * \param actorConstraint: the actor constraint that contains the operating point we want to add clusters
 ...
 * */
  ...
  /**
 * \brief Add a new operating point to the Actor Constraint.The operating point will get the specified cluster guarantee id added in his cluster list
 * \param actorConstraint: the actor constraint that contains the operating point we want to add cluster guarantee
 * \param op_id: id of the operatin point in the actor constraint
 ...
 * */
  ...
  /**
 * \brief Creates a new Agent Claim, which is intended satisying one of the operating point
 *      in the actor constraint .
 * \param parentagent: Agent handle of the parentclaim. NULL if a new Agent should be created. Use agent_claim_get_agent to get the Agent handle of an AgentOctoClaim.
 * \param actor_constr: it is an actor constraint which contains the specification of the needed resources
 * \return os::agent::AgentOctoClaim
 */
  agentclaim_t agent_claim_actor_constraint_invade(agent_t parentagent, constraints_t actor_constr);
