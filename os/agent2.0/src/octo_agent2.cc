...
int agent_get_downey_sigma(constraints_t constr) {
	if (!constr) panic("agent_get_downey_sigma: Constraints == NULL");
	AgentConstraint* Constrainte = NULL;
	if (static_cast<Constraint*>(constr)->getType() == ConstraintType::Actor) panic("This Constrainte is not a AgentConstraint");
	else Constrainte = static_cast<AgentConstraint*>(constr);
...
void agent_constr_set_downey_speedup_parameter(constraints_t constr, double average, double sigma) {
	if(!constr) panic("agent_constr_set_quantity Constraints == NULL");
	if(average <= 0) panic("Average too small");
	if(average > SHRT_MAX) panic("Average bigger than SHRT_MAX");
	if(sigma <= 0) panic("sigma too small");
	if(sigma > SHRT_MAX) panic("sigma bigger than SHRT_MAX");
...
void agent_constr_set_appnumber(constraints_t constr, int AppNr) {
	if(!constr) panic("Constraints == NULL");
	AgentConstraint* Constrainte = NULL;
	if (static_cast<Constraint*>(constr)->getType() == ConstraintType::Actor) panic("This Constrainte is not a AgentConstraint");
	else Constrainte = static_cast<AgentConstraint*>(constr);
...
void agent_constr_set_malleable(constraints_t constr, bool malleable, resize_handler_t resize_handler, resize_env_t resize_env) {
	if (!constr) panic("agent_constr_set_malleable: Constraints == NULL");
